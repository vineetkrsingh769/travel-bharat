  ...
  return (
    <div className="max-w-3xl animate-fadeIn">
      ...
      <form onSubmit={handleSubmit} className="bg-white dark:bg-[#121316] border border-[#DDD0B8]/80 dark:border-white/10 rounded-xl p-6 sm:p-8 shadow-sm space-y-6">
        <FormSection title="Identity & Location">
          ...
          {form.image_url && (
            <div className="flex items-center gap-3 p-3 bg-[#FAF5EC]/50 dark:bg-white/2 border border-[#DDD0B8]/40 dark:border-white/5 rounded-lg">
              <img src={form.image_url} alt="preview" className="h-16 w-24 object-cover rounded-md shadow-sm" onError={e => e.target.style.display='none'} />
        ...
        <FormSection title="Additional Slide Images">
          <Field label="Gallery Images (Additional Slideshow Images)">
            <div className="space-y-3">
              {(form.images || []).map((img, idx) => (
                <div key={idx} className="flex gap-2 items-center animate-fadeIn">
                  ...
                  <button
                    ...
                    className="bg-red-50 dark:bg-red-900/20 hover:bg-red-100 dark:hover:bg-red-900/40 border border-red-200 dark:border-red-500/30 text-red-650 dark:text-red-350 px-3.5 py-2.5 rounded-lg text-xs font-semibold transition-colors"
                  >
                    Remove
                  </button>
                </div>
              ...
              <button
                ...
                className="bg-[#FAF5EC] dark:bg-white/5 hover:bg-[#EDE5D4]/60 dark:hover:bg-white/10 text-ink/75 dark:text-cream/75 border border-[#DDD0B8] dark:border-white/10 px-4 py-2.5 rounded-lg text-xs font-semibold transition-colors animate-fade-in"
              >
                + Add Gallery Image
              </button>
            </div>
            ...
            {(form.images || []).filter(Boolean).length > 0 && (
              <div className="grid grid-cols-4 gap-3 p-3 bg-[#FAF5EC]/50 dark:bg-white/2 border border-[#DDD0B8]/40 dark:border-white/5 rounded-lg mt-3">
                {form.images.filter(Boolean).map((img, idx) => (
                  <div key={idx} className="relative aspect-[16/10] bg-black/10 rounded-md overflow-hidden shadow-sm border border-[#DDD0B8]/30 dark:border-white/5">
                    <img
                      src={img}
                      alt="gallery preview"
        ...
        <div className="flex gap-3 pt-5 border-t border-[#DDD0B8]/60 dark:border-white/10">
          ...
          <button type="button" onClick={() => navigate('/admin/places')} className="bg-[#FAF5EC] dark:bg-white/5 hover:bg-[#EDE5D4]/60 dark:hover:bg-white/10 border border-[#DDD0B8] dark:border-white/10 text-ink/70 dark:text-cream/75 px-6 py-2.5 rounded-lg text-sm transition-colors font-semibold">
            Cancel
          </button>
        </div>
      </form>
